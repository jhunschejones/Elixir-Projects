defmodule RumblWeb.VideoControllerTest do
  ...
  @create_attrs %{url: "http://youtu.be", title: "video", description: "not a real video"}
  ...
  describe "without a logged-in user" do
    ...
    test "authorizes actions against access by other users", %{conn: conn} do
      ...
      non_owner = user_fixture(username: "sneakypete")
  ...
  describe "with a logged-in user" do
    ...
    @tag login_as: "max"
    test "lists all users's videos on index", %{conn: conn, user: user} do
      user_video = video_fixture(user, title: "funny cats")
      other_video = video_fixture(user_fixture(username: "carl"), title: "moar cats")
